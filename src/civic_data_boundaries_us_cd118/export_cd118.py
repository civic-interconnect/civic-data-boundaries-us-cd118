...
from civic_lib_geo.us_constants import US_STATE_FIPS_TO_ABBR, get_state_dir_name
...
def export_cd118():
    ...
    if nationwide_gdfs:
        ...
        combined_gdf.to_file(nationwide_path, driver="GeoJSON")
        logger.info(f"[CD118 EXPORT] Nationwide file written to: {nationwide_path}")
        logger.info(f"[CD118 EXPORT] Nationwide file size: {nationwide_path.stat().st_size / 1e6:.2f} MB")
